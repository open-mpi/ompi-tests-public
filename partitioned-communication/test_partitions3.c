...
/* 
 * Initializes using an invalid number of partitions: Precv_init with negative partitions.
 *
...
int main(int argc, char *argv[]) {
    ...
    MPI_Count send_partitions = PARTITIONS;
    MPI_Count recv_partitions = -3; /* Bad number of partitions */
    ...
    if (0 == myrank) {
        ...
        MPI_Psend_init(message, send_partitions, COUNT, MPI_INT, dest, tag, MPI_COMM_WORLD , MPI_INFO_NULL , &request);
        ...
        fprintf(stderr, "Rank 1 initializing recv partitions to %d\n", recv_partitions);
        /* Error should occur with this call */
        CHECK_RETVAL(MPI_Precv_init(message, recv_partitions, COUNT, MPI_INT, source, tag, MPI_COMM_WORLD , MPI_INFO_NULL , &request));
