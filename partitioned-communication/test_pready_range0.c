...
/* 
 * P. 110: Definition of MPI_PREADY_RANGE. It is inclusive (line 36)
 *
 * This test just uses PREADY_RANGE instead of PREADY
 *
...
int main(int argc, char *argv[]) {
    ...
    if (0 == myrank) {
        ...
        /* Fill the sending partitions*/
        for (i = 0; i < partitions; ++i) {
            for (j = 0; j < COUNT; ++j) message[j+(COUNT*i)] = j+(COUNT*i);
            //      CHECK_RETVAL(MPI_Pready(i, request));
        }

        /* Basic Pready range use case */
        CHECK_RETVAL(MPI_Pready_range(0,partitions-1,request));
