 ...
 *  - Does have multiple partitions on send side and a single partition on recv side
 *
...
int main(int argc, char *argv[]) {
    ...
    if (0 == myrank) {
        ...
        MPI_Psend_init(message, send_partitions, send_partlength, MPI_INT, dest, tag, MPI_COMM_WORLD, info, &request);
        MPI_Start(&request);
