...
/* 
 * P. 120: definition of PARRIVED
 *
 * This test simply uses PARRIVED and confirms the data in the partition is as expected
 *
...
int main(int argc, char *argv[]) {
    ...
    if (0 == myrank) {
        ...
        /* loop until the partition of interest has arrived */
        int try = 0;
        while (!flag) {
            CHECK_RETVAL(MPI_Parrived(request, part_of_interest, &flag));
            ++try;
        }

        /* acess the partition data */
        fprintf(stderr, "Number of PARRIVED calls: %d\n", try);
        for (i=0; i < COUNT; ++i) {
            fprintf(stderr,"%d\n", message[(part_of_interest*COUNT)+i]);
        }
